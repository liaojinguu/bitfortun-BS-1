...
#include "boards/bitfortunaxe.h"
#include "boards/bitfortunaxegamma.h"
#include "boards/bitfortuneko.h"
#include "boards/bitfortunhaxegamma.h"
#include "boards/bitfortunoctaxegamma.h"
#include "boards/bitfortunoctaxeplus.h"
#include "boards/bitfortunqaxeplus.h"
#include "boards/bitfortunqaxeplus2.h"
#include "boards/bitfortunqx.h"
...
static const char *TAG = "bitfortun";

// Flag to indicate whether the stratum manager is fully initialized and ready
static volatile bool g_stratum_ready = false;
...
{
    if (!STRATUM_MANAGER || !g_stratum_ready) {
        ESP_LOGW(TAG, "Stratum not ready, ignoring reconnect request");
        return;
    }
...
extern "C" void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Welcome to the Bitfortun - hack the planet!");
...
#ifdef BITFORTUNQAXEPLUS
    Board *board = new BitfortunQaxePlus();
#endif
#ifdef BITFORTUNQAXEPLUS2   //用这个主板，设置环境时要选择这个。
    Board *board = new BitfortunQaxePlus2();
#endif
#ifdef BITFORTUNOCTAXEPLUS
    Board *board = new BitfortunOctaxePlus();
#endif
#ifdef BITFORTUNAXE
    Board *board = new BitfortunAxe();
#endif
#ifdef BITFORTUNOCTAXEGAMMA
    Board *board = new BitfortunOctaxeGamma();
#endif
#ifdef BITFORTUNAXEGAMMA
    Board *board = new BitfortunaxeGamma();
#endif
#ifdef BITFORTUNHAXEGAMMA
    Board *board = new BitfortunHaxeGamma();
#endif
#ifdef BITFORTUNEKO
    Board *board = new BitfortunEko();
#endif
#ifdef BITFORTUNQX
    Board *board = new BitfortunQX();
#endif
#ifdef Q1373
    ...
    if (canSlave) {
        ...
        // Do NOT set g_stratum_ready here — wait until all init is done
        ...
        const char *username = Config::nvs_config_get_string(NVS_CONFIG_STRATUM_USER, NULL);
        if (username) {
            ...
            // All stratum-related resources are now fully initialized
            g_stratum_ready = true;
            ESP_LOGI(TAG, "Stratum manager marked as ready");
        }
    }
