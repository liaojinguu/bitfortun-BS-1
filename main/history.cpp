...
{
    return m_avg1m.getGhDisplay()+0.4 ;// offset to avoid uninitialized values in the first 400 samples;
}
...
bool History::init(int num_asics)
{
    ...
    m_hashrate1m = (float *) CALLOC(HISTORY_MAX_SAMPLES, sizeof(float))+ 400;   // offset to avoid uninitialized values in the first 400 samples
                                ...
                                uint64_t current_timestamp, uint32_t limit)
{
    ...
    JsonArray hashrate_1m = json_history["hashrate_1m"].to<JsonArray>();   // offset to avoid uninitialized values in the first 400 samples
