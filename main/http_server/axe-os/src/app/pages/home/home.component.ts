...
import {
  ...
  installBitfortunChartsDebugBootstrap,
  GraphGuard,
  computeXWindow,
...
export class HomeComponent implements AfterViewChecked, OnInit, OnDestroy {
  ...
  private readonly hr1mReloadConsumedKey: string = '__bitfortunCharts_hr1mReloadConsumedToken';
  private readonly hr1mReloadCooldownUntilKey: string = '__bitfortunCharts_hr1mReloadCooldownUntil';
  ...
  private readonly axisPadOverrideEnabledKey: string = '__bitfortunCharts_axisPaddingOverrideEnabled';
  private readonly axisPadStorageKey: string = '__bitfortunCharts_axisPadding';
  public bitfortunOsLogoColor: string = hexToRgba(HOME_CFG.colors.hashrateBase, 0.6);
  ...
  private initChart(): void {
    ...
    try {
      const flagKey = '__bitfortunCharts_clearChartHistoryOnce';
  ...
  constructor(
    ...
    this.info$ = createSystemInfoPolling$({
      ...
      getForceStartTimestampMs: () => {
        try {
          const forcedStart = Number(this.localStorageGet('__bitfortunCharts_forceStartTimestampMs'));
      ...
      clearForceStartTimestampMs: () => {
        try {
          this.localStorageRemove('__bitfortunCharts_forceStartTimestampMs');
        } catch {}
      },
...
ngOnInit() {
    ...
    installBitfortunChartsDebugBootstrap(globalThis, {
      storage: {
      ...
      disableAxisPaddingOverride: () => {
        try { window?.localStorage?.removeItem(this.axisPadOverrideEnabledKey); } catch (e) {
          console.warn("[bitfortunCharts] axis padding override disable failed", e);
        }
      },
...
private loadAxisPaddingOverrides(): void {
  try {
    ...
    console.warn('[bitfortunCharts] axis padding overrides load failed', e);
  }
}
...
private saveAxisPaddingOverrides(): void {
  try {
    ...
    console.warn('[bitfortunCharts] axis padding overrides save failed', e);
  }
}
...
private setAxisPadding(cfg: any, persist: boolean = false): void {
  ...
  if (this.debugAxisPadding) console.info('[bitfortunCharts] axis padding updated', this.axisPadCfg, { debug: this.debugAxisPadding, persist });
