...
chart/
    ...
    home.debug-hooks.ts             # __bitfortunCharts wiring
...
- **Debug**: `chart/home.debug-hooks.ts`
  - Global commands (`__bitfortunCharts`) and storage keys.
...
`chart/home.debug-hooks.ts` installs `globalThis.__bitfortunCharts`.

### Enable hooks
Some builds only install the lightweight bootstrap. In that case, run:

```js
__bitfortunCharts.enable(true)  // persist across reloads
// or:
__bitfortunCharts.enable()
```

You can inspect available commands via:

```js
__bitfortunCharts.list()
__bitfortunCharts.help()
```
...
__bitfortunCharts.clearChartHistoryNow()

// Clear history once (persists a flag + seeds timestamps; reload to apply)
__bitfortunCharts.clearChartHistoryOnce(); location.reload()

// Force a render tick of the history-drain loop
__bitfortunCharts.flushHistoryDrainRender()

// Restart device (optional hook; may require a TOTP depending on backend configuration)
__bitfortunCharts.restart()
__bitfortunCharts.restart("123456")
```

### Storage keys (typical)
- `__bitfortunCharts_debugMode` (debug active)
- `__bitfortunCharts_axisPaddingOverrideEnabled` / `__bitfortunCharts_axisPadding` (axis padding overrides)
- `__bitfortunCharts_clearChartHistoryOnce` (delete once)
