                                ...
                                bool hasEth)
{
    ...
    if (hasEth) {
        err = m_eth.init();   // 先 init ETH
        if (err != ESP_OK) {
            ESP_LOGW(TAG_NET, "ETH init failed: %s", esp_err_to_name(err));
        }
    }

    m_wifiStaNetif = wifi_init(wifi_ssid, wifi_pass, hostname);  // 后 init WiFi
    return ESP_OK;
}
...
void NetworkManager::onEthGotIp()
{
    ...
    if (!m_wifiDisabledBecauseEth) {
        ...
        // === 关键修复：增加延迟并检查状态 ===
        vTaskDelay(pdMS_TO_TICKS(500));  // 给 WiFi 事件处理一些时间

        esp_err_t err = esp_wifi_disconnect();
        if (err != ESP_OK && err != ESP_ERR_WIFI_NOT_STARTED) {
            ESP_LOGW(TAG_NET, "esp_wifi_disconnect failed: %s", esp_err_to_name(err));
        }
        ...
        err = esp_wifi_stop();
        if (err != ESP_OK && err != ESP_ERR_WIFI_NOT_STARTED) {
            ESP_LOGW(TAG_NET, "esp_wifi_stop failed: %s", esp_err_to_name(err));
        }
