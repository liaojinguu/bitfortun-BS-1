...
#include "drivers/bitfortunaxe/DS4432U.h"
#include "drivers/bitfortunaxe/EMC2101.h"
#include "drivers/bitfortunaxe/INA260.h"
#include "drivers/bitfortunaxe/adc.h"

static const char* TAG="bitfortunaxe";
...
BitfortunAxe::BitfortunAxe() : Board() {
    m_deviceModel = "BitfortunAxe";
 ...
 */
uint8_t BitfortunAxe::ds4432_tps40305_bitaxe_voltage_to_reg(float vout)
{
    float change;
    uint8_t reg;
...
void BitfortunAxe::shutdown() {
    setVoltage(0.0);

    Board::shutdown();
}

bool BitfortunAxe::initAsics()
{
...
void BitfortunAxe::requestBuckTelemtry() {
    // NOP
}

bool BitfortunAxe::setVoltage(float core_voltage)
{
...
void BitfortunAxe::setFanSpeedCh(int channel, float perc) {
    if (channel == 0) {
        EMC2101_set_fan_speed(perc);
    }
}

void BitfortunAxe::setFanPolarity(bool invert) {
    EMC2101_set_fan_polarity(invert);
}

void BitfortunAxe::getFanSpeedCh(int channel, uint16_t* rpm) {
    *rpm = EMC2101_get_fan_speed();
}

float BitfortunAxe::getTemperature(int index) {
    if (index > 0) {
        return 0.0f;
    }
...
float BitfortunAxe::getVRTemp() {
    return 0.0f;
}

float BitfortunAxe::getVin() {
    return INA260_read_voltage() / 1000.0;
}

float BitfortunAxe::getIin() {
    return INA260_read_current() / 1000.0;
}

float BitfortunAxe::getPin() {
    return INA260_read_power() / 1000;
}

float BitfortunAxe::getVout() {
    return ADC_get_vcore() / 1000.0;
}

float BitfortunAxe::getIout() {
    return 0.0;
}

float BitfortunAxe::getPout() {
    return 0.0;
}

bool BitfortunAxe::selfTest(){
    //Test Core Voltage
