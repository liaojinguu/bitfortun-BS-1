...
static const char* TAG="bitfortunqaxe+";

#define VR_TEMP1075_ADDR   0x1

BitfortunQaxePlus::BitfortunQaxePlus() : Board() {
    m_deviceModel = "BitfortunQAxe+";
    ...
    m_theme = new ThemeBitfortunqaxeplus();
#endif
...
bool BitfortunQaxePlus::initBoard()
{
    Board::initBoard();
...
void BitfortunQaxePlus::shutdown() {
    setVoltage(0.0);
...
void BitfortunQaxePlus::setAsicReset(bool state) {
    gpio_set_level(BM1368_RST_PIN, !!state);
}

bool BitfortunQaxePlus::initAsics()
{
...
void BitfortunQaxePlus::requestBuckTelemtry() {
    m_tps->status();
}

void BitfortunQaxePlus::requestChipTemps() {
    if (!m_asics) {
        return;
    }
...
void BitfortunQaxePlus::LDO_enable()
{
    ESP_LOGI(TAG, "Enabled LDOs");
    gpio_set_level(LDO_EN_PIN, 1);
}

void BitfortunQaxePlus::LDO_disable()
{
    ESP_LOGI(TAG, "Disable LDOs");
    gpio_set_level(LDO_EN_PIN, 0);
}

void BitfortunQaxePlus::VREG_enable()
{
    gpio_set_level(TPS53647_EN_PIN, 1);
}

void BitfortunQaxePlus::VREG_disable()
{
    gpio_set_level(TPS53647_EN_PIN, 0);
}

bool BitfortunQaxePlus::setVoltage(float core_voltage)
{
...
void BitfortunQaxePlus::setFanSpeedCh(int channel, float perc) {
    EMC2302_set_fan_speed(channel, perc);
}

void BitfortunQaxePlus::getFanSpeedCh(int channel, uint16_t* rpm) {
    EMC2302_get_fan_speed(channel, rpm);
}

void BitfortunQaxePlus::setFanPolarity(bool invert) {
    EMC2302_set_fan_polarity(invert);
}

// return the number of asic temp measuring sensors
// skips the VR temp sensor
int BitfortunQaxePlus::detectNumTempSensors() {
    int found = 0;
    for (int i = 0; i < 4; i++) {
...
float BitfortunQaxePlus::getTemperature(int index) {
...
float BitfortunQaxePlus::getVRTemp() {
    return TMP1075_read_temperature(1);
}

float BitfortunQaxePlus::getVRTempInt() {
    return m_tps->get_temperature();
}

float BitfortunQaxePlus::getVin() {
    return m_tps->get_vin();
}

float BitfortunQaxePlus::getIin() {
    return m_tps->get_iin();
}

float BitfortunQaxePlus::getPin() {
    return m_tps->get_pin();
}

float BitfortunQaxePlus::getVout() {
    return m_tps->get_vout();
}

float BitfortunQaxePlus::getIout() {
    return m_tps->get_iout();
}

float BitfortunQaxePlus::getPout() {
    return m_tps->get_pout();
}

Board::Error BitfortunQaxePlus::getFault(uint32_t *status) {
    *status = 0x00000000;
...
bool BitfortunQaxePlus::isCanSlave()
{
...
bool BitfortunQaxePlus::selfTest(){
    //Test Core Voltage
