...
bool TPS53667::init(int num_phases, int imax, float ifault)
{
    ...
    // BITFORTUNOCTAXEGAMMA: Disable phase shedding to force all 6 phases active at all times
    ...
    ESP_LOGI(TAG, "Phase shedding DISABLED (0x89) - forcing all 6 phases ON (BitfortunOctaxeGamma)");
#else
    // Other boards (BitfortunEKO, etc.): Enable dynamic phase shedding for efficiency
