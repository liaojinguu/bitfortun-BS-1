...
#include "drivers/bitfortunaxe/TPS546.h"
...
static const char* TAG="bitfortunaxeGamma";

#define MAX(a,b) ((a)>(b)?(a):(b))

BitfortunaxeGamma::BitfortunaxeGamma() : BitfortunAxe() {
    m_deviceModel = "BitfortunAxeGamma";
    m_miningAgent = "BitfortunAxe";
    m_asicModel = "BM1373";
    m_version = 200;
    m_asicCount = 1;
    ...
    m_theme = new ThemeBitfortunaxegamma();
#endif
...
bool BitfortunaxeGamma::initBoard()
{
    Board::initBoard();
...
void BitfortunaxeGamma::shutdown() {
    setVoltage(0.0);

    Board::shutdown();
}

bool BitfortunaxeGamma::initAsics() {
...
bool BitfortunaxeGamma::setVoltage(float core_voltage)
{
...
float BitfortunaxeGamma::getTemperature(int index) {
...
float BitfortunaxeGamma::getVRTemp() {
...
float BitfortunaxeGamma::getVin() {
    return TPS546_get_vin();
}

float BitfortunaxeGamma::getIin() {
    float vin = getVin();
    if (!vin) {
        return 0.0f;
    }
...
float BitfortunaxeGamma::getPin() {
    return (TPS546_get_vout() * TPS546_get_iout()) + GAMMA_POWER_OFFSET;
}

float BitfortunaxeGamma::getVout() {
    return ADC_get_vcore() / 1000.0;
}

float BitfortunaxeGamma::getIout() {
    return TPS546_get_iout();
}

float BitfortunaxeGamma::getPout() {
    return getPin();
}
