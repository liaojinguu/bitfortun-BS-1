...
static const char *TAG = "bitfortunoctaxegamma";

BitfortunOctaxeGamma::BitfortunOctaxeGamma()
    : BitfortunQaxePlus2(), m_tmp451{// ASICs 0–3: GPIO2=A0, GPIO12=A1, addr 0x4c
                                Tmp451Mux(GPIO_NUM_2, GPIO_NUM_12, 0x4c),
                                // ASICs 4–7: same MUX select lines, addr 0x4e
                                Tmp451Mux(GPIO_NUM_2, GPIO_NUM_12, 0x4e)}
{
    m_deviceModel = "BitfortunOCTAXE-γ";
    ...
    m_theme = new ThemeBitfortunoctaxegamma();
#endif
    ...
    if (isTPS53667) {
        ...
        m_absMaxAsicFrequency = 5000; // Absolute max for manual input (danger zone)
...
bool BitfortunOctaxeGamma::initBoard()
{
    // Call parent initBoard() (VR init, ASIC power-on, etc.)
    bool ret = BitfortunQaxePlus::initBoard();
...
void BitfortunOctaxeGamma::requestChipTemps()
{
...
float BitfortunOctaxeGamma::getVRTemp()
{
    // Get temperature from parent implementation
    float vrTemp = BitfortunQaxePlus::getVRTemp();
