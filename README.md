...
```bash
# clone repository
git clone https://github.com/shufps/ESP-Miner-BitfortunQAxePlus

# change into the cloned repository
cd ESP-Miner-BitfortunQAxePlus
...
```
bitaxetool --config ./config.cvs --firmware esp-miner-factory-BITFORTUNQAXEPLUS-v1.0.10.bin
...
export BOARD="BITFORTUNQAXEPLUS2"
...
```bash
./docker/bitaxetool.sh --config config.cvs --firmware esp-miner-factory-BITFORTUNQAXEPLUS-v1.0.10.bin -p /dev/ttyACM0
```
...
./merge_bin.sh bitfortunqaxe+.bin

bitaxetool --config config.cvs --firmware esp-miner-factory-bitfortunqaxe+.bin  -p /dev/ttyACM0
```
...
# merge all partitions including config into a single binary
./merge_bin_with_config.sh bitfortunqaxe+.bin

# flash using esptool
esptool.py --chip esp32s3 -p /dev/ttyACM0 -b 460800 \
  --before=default_reset --after=hard_reset write_flash \
  --flash_mode dio --flash_freq 80m --flash_size 16MB 0x0 bitfortunqaxe+.bin
```
